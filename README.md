...
- Automatic watchers reload ledger and pool configuration when files change
...
- **Unified pool registry** refreshed nightly from `data/pools.toml` using
  ...
  `POOLS_FILE`. A file watcher reloads changes automatically, logs the path and verifies the file exists.
...
effect without redeploying; missing files log an error and duplicate watchers
are ignored. Integration tests set the variables automatically for the local
environment.
