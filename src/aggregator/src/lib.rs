...
async fn calculate_holdings(principal: Principal) -> (Vec<Holding>, Vec<HoldingSummary>) {
    let (ledger_filter, dex_filter) = match user_settings::get(&principal) {
        Some(user_settings::UserSettings { ledgers, dexes }) => (ledgers, dexes),
        None => (None, None),
    };
    let (ledger, neuron, dex) = futures::join!(
        ledger_fetcher::fetch_filtered(principal, ledger_filter.as_ref()),
        neuron_fetcher::fetch(principal),
        dex_fetchers::fetch_filtered(principal, dex_filter.as_ref())
    );
...
pub(crate) fn summarise(holdings: &[Holding]) -> Vec<HoldingSummary> {
